...
class LoadUnloadReload(commands.Cog):
    ...
    async def loading_behaviour(self, ctx, meth, ext, success_message):
        try:
            ...
            try:
                meth(f"{self.client.config.relative_cogs_dir}.{ext}")
            except (commands.ExtensionNotFound, commands.ExtensionNotLoaded):
                return await ctx.send("The extension could not be found in either the current working directory or relative "
                               "cogs directory")
        await ctx.send(success_message)

    @commands.command(help="load the given extension, developer only")
    async def load(self, ctx, ext):
        await self.loading_behaviour(ctx, self.client.load_extension, ext, f'{ext} loaded')

    @commands.command(help="unload the given extension, developer only")
    async def unload(self, ctx, ext):
        await self.loading_behaviour(ctx, self.client.unload_extension, ext, f'{ext} unloaded')

    @commands.command(help="reload the given extension, developer only")
    async def reload(self, ctx, ext):
        await self.loading_behaviour(ctx, self.client.reload_extension, ext, f'{ext} reloaded')

    @commands.command(name="creload", aliases=['reloadc', 'ureload', 'reloadu'], help="reload the config and utils, developer only")
